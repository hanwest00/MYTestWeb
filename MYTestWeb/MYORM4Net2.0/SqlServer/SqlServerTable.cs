...
namespace MYORM.SqlServer
{
    public class SqlServerTable<T> : MYTableBase<T>, Interfaces.ITable<T> where T : MYORM.MYItemBase
    {
        ...
        public override void Insert(T item)
        {
            ...
            try
            {
                foreach (System.Reflection.PropertyInfo s in TableType.GetProperties())
                {
        ...
        public override void Delete(IList<MYDBCondition> conds)
        {
            ...
            try
            {
                ...
                {
                    sb.Append(" where 1 = 1 ");
                    foreach (MYDBCondition s in conds) sb.Append(s.ToQueryString());
        ...
        public override void Where(IList<MYDBCondition> conds, params DbParameter[] sqlParams)
        {
            if (conds != null)
            {
                ...
                foreach (MYDBCondition s in conds) if (s != null) whereQuery.Append(s.ToQueryString());
            }
            if (sqlParams != null)
        ...
        public override void Join(params MYDBQJoin[] joinTables)
        {
            if (joinTables != null)
            {
                ...
                foreach (MYDBQJoin s in joinTables) joinQuery.Append(s.ToQueryString());
            }
        }
        ...
        /// <param name="conds">字段是否为保留字如count as max abs等</param>
        /// <returns></returns>
        public override object SelectFeild(string fieldName, IList<MYDBCondition> conds)
        {
            ...
            if (conds != null && conds.Count > 0) foreach (MYDBCondition s in conds) sb.Append(s.ToQueryString());
